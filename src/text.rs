...
fn test_english() {
    ...
    text.count_all("this is a test to see that real english is closer to the english \
                    distribution than some gibberish of the same length");
    gibberish.count_all("guvf vf n grfg gb frr gung erny ratyvfu vf pybfre gb gur ratyvfu \
                         qvfgevohgvba guna fbzr tvoorevfu bs gur fnzr yratgu");
    assert!(text.dist(&en) < (gibberish.dist(&en) - 0.5));
}
